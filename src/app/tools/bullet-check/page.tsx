...
const QUANTIFIER_RE = /(\d+(\.\d+)?\s*%|\$\s*\d+|[0-9]+(?:[,.][0-9]+)*\+?|\b(?:million|thousand|k|m|b)\b)/i;
...
export default function BulletCheckPage() {
  ...
  const avg = bullets.length === 0 ? 0 : Math.round(bullets.reduce((s, b) => s + b.score, 0) / bullets.length);
